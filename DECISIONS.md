...
## D-031：V0.4.2 — 文件关系引擎架构

**决策**：关系引擎采用三层架构：Fingerprint → Similarity → Relationship Graph。

**理由**：

- Fingerprint 层：从已有 Content Summary + Classification Result 派生，不重复读取文件内容
- Similarity 层：纯规则驱动，多维度加权（主题 0.4 + 关键词 0.25 + 实体 0.15 + 路径 0.1 + 文件名 0.1）
- Relationship Graph 层：候选过滤 → 相似度计算 → 连通分量分组 → 分组建议

**Trade-off**：

- 规则驱动不如 ML 准确，但可解释性强，每条边都有 evidence
- 候选过滤基于启发式，可能漏掉一些弱关系（但性能优先）
- 不依赖 LLM，纯本地计算
...
**决策**：使用 `isCandidatePair()` 快速过滤，避免全量 N² 比较。候选条件：同主题 / 同目录 / 关键词重叠 / 实体重叠 / 文件名高度相似（>85%）。

**理由**：

- 100 个文件全量比较 = 4950 对，候选过滤后降至 ~2450 对（减少 50%）
- 文件名相似度阈值设为 0.85，避免 `file_0` vs `file_1` 这类人工命名误触发
- 同主题 + 同目录是 strongest 信号，优先匹配

**Trade-off**：

- 可能漏掉一些弱关系（如不同目录但共享实体的文件，如果实体不重叠）
- 文件名相似度阈值 0.85 是经验值，可能不适用于所有场景

**状态**：V0.4.2 已实施。

---

## D-034：V0.4.2.1 — 主题相同不能单独建立关系

**决策**：主题匹配（theme === theme）不能单独作为关系证据。必须同时有实体重叠、关键词重叠或文件名高度相似，主题匹配才计分（权重 0.15）。

**理由**：

- 项目A和项目B都是 theme=项目，但实体完全不同，不应产生关系
- V0.4.2 中主题权重 0.4 是最高权重，导致"两个文件都是项目主题"就产生假关系
- 降权到 0.15 并要求额外证据后，多项目同主题的假阳性降至 0

**Trade-off**：

- 弱关系（只有主题相同）会被漏掉，但这是正确的 — 主题相同不等于同一项目
- 实体匹配（0.35）成为最强信号，更可靠

**状态**：V0.4.2.1 已实施。

---

## D-035：V0.4.2.1 — Group Cohesion 替代简单连通分量

**决策**：使用 Group Cohesion 检测替代 BFS 连通分量。要求：强核心边（score >= 0.4）+ 实体一致性（候选节点必须与组核心实体有交集）+ 边密度评估。

**理由**：

- 简单 BFS 连通分量会导致链式污染：A-B 强连接，B-C 弱连接，C 也会被纳入组
- Bridge file（同时提到项目A和项目B）会通过实体共享连接两个项目，导致项目合并
- 实体一致性检查确保只有共享核心实体的文件才能加入组

**Trade-off**：

- 更严格的分组标准可能导致一些弱关系被漏掉
- 但显著提高了 Precision（从 0.353 升至 1.0）

**状态**：V0.4.2.1 已实施。

---

## D-036：V0.4.2.1 — 实体提取过滤通用功能词

**决策**：从文件名提取实体时，过滤通用功能词（需求/架构/设计/测试/部署/接口等）和短缩写（ASCII < 3 字符如 "PR" / "AI" / "UI"）。

**理由**：

- 文件名中的"需求"、"架构"等词是功能描述，不是项目实体
- "PR"、"AI" 等 2-3 字符大写缩写是通用前缀，不是项目标识
- 这些词在不同项目文件中频繁出现，会导致假实体共享

**Trade-off**：

- 可能漏掉一些有效的实体（如英文单词 "API"、"UI"）
- 但这些词在上下文中通常有更明确的项目实体伴随
...
**决策**：关系引擎使用 `getFileId(file)` 统一标识，优先 `file.path`，回退 `file.name`。

**理由**：

- 生产环境 FileEntry 有 `path` 字段
- 测试环境可能只有 `name` 字段
- 避免在 `undefined` 键上构建图导致分组错误
